...
class SupportFilesCar:
    ...
    def __init__(self):
        ''' Load the constants thatdo not change'''
        ...
        lr=1.6
        Ts=0.02
        track=1.5  # Track width for 4-wheel model visualization
        ...
        Q=np.matrix('2 0;0 10') # weights for outputs (all samples, except the last one)
        S=np.matrix('2 0;0 10') # weights for the final horizon period outputs
        R=np.matrix('6') # weights for inputs (only 1 input in our case)

        outputs=2 # number of outputs
        hz = 50 # horizon prediction period
        ...
        self.constants={'m':m, 'Iz':Iz, 'Caf':Caf, 'Car':Car, 'lf':lf, 'lr':lr,\
            ...
            'Kp_Y':Kp_Y, 'Kd_Y':Kd_Y, 'Ki_Y':Ki_Y, 'trajectory':trajectory, 'track':track}
    ...
    def trajectory_generator(self,t,r,f):
        ...
        if trajectory==1:
            ...
            # Closed-loop oval track (as in image)
            loop_len = len(t)
            theta = np.linspace(0, 2 * np.pi, loop_len)
            a, b = 150, 100
            x = a * np.cos(theta) + 150
            y = b * np.sin(theta) + 150
            # Add small sinusoidal bumps for realism
            y += 10 * np.sin(3 * theta)
            x += 5 * np.sin(2 * theta)
        else:
    ...
    def plot_simulation_results(self, t, X_ref, Y_ref, statesTotal, UTotal):
        """Generate 4 simulation result plots for GUI display."""
        fig, axs = plt.subplots(4, 1, figsize=(9, 11))
        axs[0].plot(t, statesTotal[:, 0], 'b', label='x_dot [m/s]')
        axs[0].set_ylabel("x_dot [m/s]")
        axs[0].grid(True)
        axs[0].legend()

        axs[1].plot(t, UTotal, 'r', label='delta [rad]')
        axs[1].set_ylabel("delta [rad]")
        axs[1].grid(True)
        axs[1].legend()

        axs[2].plot(t, statesTotal[:, 1], 'purple', label='Psi [rad]')
        axs[2].set_ylabel("Psi [rad]")
        axs[2].grid(True)
        axs[2].legend()

        axs[3].plot(X_ref, Y_ref, 'b', label='Reference path')
        axs[3].plot(X_ref, statesTotal[:, 3], '--r', label='Car path')
